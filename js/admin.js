...
function renderAdminDetail(metrics){
  ...
  if(adminDetailType === 'user'){
    ...
    const orderRows = orders.map(o => '<div class="flex justify-between gap-3 py-3 border-b border-slate-100"><span class="text-xs font-bold">'+esc(o.id)+'</span><span class="text-xs text-slate-500">'+esc(o.orderStatus||o.status||'New')+' · '+fmt(o.total||0)+'</span></div>').join('') || '<div class="text-sm text-slate-400">No orders recorded for this user.</div>';
