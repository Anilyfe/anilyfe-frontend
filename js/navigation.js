...
document.addEventListener('click', e=>{
  ...
  switch(act){
    ...
    case 'admin-announcement-delete': {const rows=LS.get('announcements',[]).filter(a=>a.id!==t.dataset.id);LS.set('announcements',rows);route();break;}
...
function submitSupportRequest(form){
  const f=new FormData(form),settings=LS.get('marketplaceSettings',{});
  const ticket={id:uid('TKT'),customer:String(f.get('name')||'').trim(),email:String(f.get('email')||'').trim(),category:String(f.get('topic')||'Other'),message:String(f.get('message')||'').trim(),priority:'Normal',status:'Open',date:new Date().toISOString()};
  if(!ticket.customer||!ticket.email||!ticket.message){toast('Complete your name, email and message.','alert-circle');return;}
  const rows=LS.get('tickets',[]);rows.unshift(ticket);LS.set('tickets',rows);window.adminNotificationService?.add({type:'support',title:'New support request',message:`${ticket.customer} · ${ticket.category}`,ticketId:ticket.id});toast('Support request saved in the admin inbox.','mail');
  const to=settings.supportEmail||settings.adminEmail||''; if(to) window.location.href=`mailto:${to}?subject=${encodeURIComponent('ANILyfe Support: '+ticket.category)}&body=${encodeURIComponent('Name: '+ticket.customer+'\nEmail: '+ticket.email+'\n\n'+ticket.message)}`;
}
async function publishAdminAnnouncement(form){
  const f=new FormData(form), file=f.get('image'); let image='';
  if(file&&file.name){if(!file.type.startsWith('image/')){toast('Announcement image must be an image.','alert-circle');return;}if(file.size>2*1024*1024){toast('Announcement image must be 2 MB or smaller.','alert-circle');return;}image=await new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(r.result);r.onerror=reject;r.readAsDataURL(file);});}
  const text=String(f.get('text')||'').trim(); if(!text){toast('Announcement text is required.','alert-circle');return;}
  const rows=LS.get('announcements',[]);rows.unshift({id:uid('ANN'),text,image,link:String(f.get('link')||'').trim(),active:true,createdAt:new Date().toISOString(),createdBy:currentAdmin()?.id||null});LS.set('announcements',rows);toast('Announcement published.','megaphone');route();
}
...
async function submitSellerVerification(formEl){
  try {
    ...
    const files=[];
    for(const name of ['cacDocument','identityDocument','addressDocument']){ const file=f.get(name); if(file && file.name){ files.push({name:file.name,size:`${Math.max(1,Math.round(file.size/1024))} KB`,uploadedAt:new Date().toISOString().slice(0,10),status:'Pending Review'}); } }
    const data={legalBusinessName:String(f.get('legalBusinessName')||'').trim(),registrationType:String(f.get('registrationType')||'').trim(),taxIdentificationNumber:String(f.get('taxIdentificationNumber')||'').trim(),directorName:String(f.get('directorName')||'').trim(),nationalIdType:String(f.get('nationalIdType')||'').trim(),nationalIdNumber:String(f.get('nationalIdNumber')||'').trim(),registeredAddress:String(f.get('registeredAddress')||'').trim(),submittedDocuments:files};
    ...
    toast('Verification submitted for admin review.','shield-check');
    route();
  } catch(err){ toast(err.message||'Unable to submit verification.','alert-triangle'); }
}
