...
function viewMarketplace(){
  ...
  const announcement=(LS.get('announcements',[])||[]).find(a=>a.active!==false && !(LS.get('announcementDismissed',[])||[]).includes(a.id));
  return `
  <div class="min-h-screen bg-[#F6FCFF]">
    <!-- top bar -->
    ${announcement ? `<div class="bg-[#081F5C] text-white"><div class="max-w-[1400px] mx-auto px-4 py-2 flex items-center gap-3 text-xs"><i data-lucide="megaphone" style="width:14px;height:14px;color:#E9B949"></i>${announcement.image ? `<img src="${announcement.image}" class="w-7 h-7 rounded-md object-cover" alt="">` : ""}<span class="flex-1 font-bold">${esc(announcement.text)}</span>${announcement.link ? `<a href="${esc(announcement.link)}" target="_blank" rel="noopener" class="text-[#E9B949] font-extrabold">Open</a>` : ""}<button data-action="dismiss-announcement" data-id="${announcement.id}" class="text-white/70">✕</button></div></div>` : ""}
    ...
    <div class="max-w-[1400px] mx-auto px-4 py-6 grid grid-cols-1 lg:grid-cols-[250px_1fr_300px] gap-6">
      ...
      <main class="min-w-0">
        ...
        <div class="relative rounded-2xl overflow-hidden mb-7 reveal" style="background:linear-gradient(115deg,#081F5C 0%,#0B2A7A 55%,#334EAC 100%)">
          <img src="" class="absolute inset-0 w-full h-full object-cover opacity-30" onerror="this.remove()" alt=""/>
