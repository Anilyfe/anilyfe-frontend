...
function checkoutNow(){
  ...
  for(const [sellerId, items] of groups){
    ...
    const marketplaceSettings = window.getMarketplaceSettings ? window.getMarketplaceSettings() : LS.get('marketplaceSettings', {commission:15});
    const commissionRate = Math.max(0, Math.min(100, Number(marketplaceSettings.commission ?? 15))) / 100;
    const commissionAmount = Math.round((subtotal + ship) * commissionRate);
    const sellerEarnings = Math.max(0, (subtotal + ship) - commissionAmount);
    const order = { id:'ANL-'+Date.now().toString(36).toUpperCase()+'-'+Math.random().toString(36).slice(2,5).toUpperCase(), buyerId:u.id, buyer:{id:u.id,name:u.name,email:u.email}, sellerId, items:items.map(x=>({productId:x.product.id,productName:x.product.name,sku:x.product.sku||'',quantity:x.qty,unitPrice:x.unit,lineTotal:x.lineTotal})), subtotal, shipping:ship, total:subtotal+ship, commissionRate:commissionRate*100, commissionAmount, sellerEarnings, paymentStatus:'Pending Payment', orderStatus:'New', createdAt:new Date().toISOString(), timeline:[{status:'New',label:'Order created',timestamp:new Date().toISOString(),note:'Awaiting payment processing.'}] };
