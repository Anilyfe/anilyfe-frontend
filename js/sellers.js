    ...
    <main class="max-w-5xl mx-auto px-4 py-10">
      <div class="flex items-center gap-5 mb-8 reveal">
        <div class="relative w-20 h-20 shrink-0">
          <div class="w-20 h-20 rounded-3xl bg-gradient-to-br from-[#334EAC] to-[#081F5C] text-white flex items-center justify-center font-display font-extrabold text-3xl shadow-xl overflow-hidden">
            ${u.profilePicture ? `<img src="${u.profilePicture}" alt="Profile photo" class="w-full h-full object-cover" />` : esc((u.name || 'U')[0].toUpperCase())}
          </div>
        </div>
        <div>
          ...
          <div class="flex flex-wrap gap-2 mt-2 text-xs">
            ...
            <span class="badge !bg-[#E7F1FF] !text-[#334EAC]"><i data-lucide="calendar" style="width:11px;height:11px"></i> Joined ${u.createdAt ? new Date(u.createdAt).toLocaleDateString(undefined,{day:'numeric',month:'short',year:'numeric'}) : '—'}</span>
          </div>
        </div>
      </div>
      ...
      <div class="grid md:grid-cols-3 gap-5 reveal">
        <div class="card p-6 md:col-span-2">
          ...
          <div class="mb-5 p-4 rounded-2xl border border-[#D0E3FF] bg-[#F8FAFC] flex flex-col sm:flex-row sm:items-center gap-4">
            <div class="w-16 h-16 rounded-2xl overflow-hidden bg-[#E7F1FF] border border-[#D0E3FF] flex items-center justify-center text-[#334EAC] font-extrabold text-xl">${u.profilePicture ? `<img src="${u.profilePicture}" alt="Profile photo" class="w-full h-full object-cover" />` : esc((u.name || 'U')[0].toUpperCase())}</div>
            <div class="flex-1"><div class="font-bold text-sm">Profile photo</div><div class="text-xs text-[#708BD1] mt-1">Upload a photo for your buyer profile.</div></div>
            <label class="btn btn-primary !px-3 !py-2 !text-xs cursor-pointer">Upload photo<input type="file" accept="image/*" data-profile-image class="hidden" /></label>
          </div>
      ...
      <div class="card p-8 max-w-xl reveal">
        ...
        <p class="text-sm text-[#5a6a9c] mb-6">${s.status==='pending'?'The primary administrator is reviewing your application. Seller Dashboard and product publishing remain locked until approval.':'Manage your catalog, stock and listings from your dashboard.'}</p>
...
async function viewSellerStore(idOrSlug){
  let s = null;
  let products = [];
  const slugParam = (idOrSlug || '').toLowerCase();

  // Try storeService first
  if (window.storeService) {
    try {
      const sf = await window.storeService.getStorefront(idOrSlug || null);
      if (sf.profile && (!idOrSlug || slugParam === sf.profile.slug)) {
        s = sf.profile;
  ...
  // Direct fallback to the exact requested seller only. Never substitute another seller.
  if (!s && idOrSlug) {
    const sellers = LS.get('sellers', []);
    s = sellers.find(x => (x.slug && x.slug.toLowerCase() === slugParam) || (x.id && x.id.toLowerCase() === slugParam));
    if (s && s.status === 'approved') {
      products = LS.get('products', []).filter(p => p.sellerId === s.id && ['Published','Approved','Out of Stock'].includes(p.approvalStatus || p.status));
    }
  }
  ...
  const isFounding = Boolean(s.foundingSeller || s.foundingSellerNumber || s.foundingSellerActive);
  const bannerUrl = s.banner || '';
  const logoUrl = s.logo || '';
  const cart = LS.get('cart', []);
  const wish = LS.get('wishlist', []);
  const storeOrders = LS.get('orders', []).filter(o => o.sellerId === s.id);
  const fulfilledOrders = storeOrders.filter(o => ['Delivered','Completed'].includes(o.orderStatus || o.status));
    ...
    <main class="max-w-[1400px] mx-auto px-4 sm:px-6 pt-6">
        ...
        <div class="relative px-6 sm:px-8 pb-8 pt-0 -mt-16 sm:-mt-20 flex flex-col md:flex-row items-start md:items-end justify-between gap-6 text-white">
          <div class="flex flex-col sm:flex-row items-start sm:items-end gap-5">
            <div class="relative w-24 h-24 sm:w-28 sm:h-28 rounded-3xl overflow-hidden border-4 border-white bg-slate-900 shadow-xl shrink-0">
              <img src="${logoUrl}" alt="${esc(s.storeName || s.businessName)}" class="w-full h-full object-cover"  />
            </div>
            ...
            <div>
              ...
              <div class="flex flex-wrap items-center gap-3 mt-3 text-xs text-blue-200">
                <span class="flex items-center gap-1"><i data-lucide="map-pin" style="width:13px;height:13px"></i> ${esc(s.city || '—')}, ${esc(s.state || '—')}, Nigeria 🇳🇬</span>
                <span>·</span>
                <span class="flex items-center gap-1"><i data-lucide="calendar" style="width:13px;height:13px"></i> Merchant since June 2026</span>
                <span>·</span>
                <span class="flex items-center gap-1 text-amber-300 font-bold"><i data-lucide="star" style="width:13px;height:13px;fill:#FCD34D;color:#FCD34D"></i> ${Number(s.rating || 0)} (${Number(s.reviewCount || 0)} reviews)</span>
              </div>
            </div>
          </div>
          ...
          <div class="flex items-center gap-2.5 shrink-0 w-full sm:w-auto">
            <button class="btn btn-outline !bg-white/10 !text-white !border-white/25 hover:!bg-white/20 !py-2.5 !px-4 text-xs font-bold flex-1 sm:flex-initial" data-action="copy-store-url" data-slug="${s.slug || ''}">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-3.5 mt-6 reveal">
        <div class="card p-4 text-center">
          ...
          <div class="font-display font-extrabold text-2xl text-[#081F5C] mt-1">${Number(s.sales || 0)}</div>
          <div class="text-[10px] text-slate-500 font-bold mt-0.5">${fulfilledOrders.length ? 'Completed marketplace orders' : 'No completed orders yet'}</div>
        </div>
        <div class="card p-4 text-center">
          ...
          <div class="font-display font-extrabold text-2xl text-[#081F5C] mt-1 flex items-center justify-center gap-1">
            <span>${Number(s.rating || 0)}</span>
            <i data-lucide="star" style="width:18px;height:18px;color:#E9B949;fill:#E9B949"></i>
          </div>
          <div class="text-[10px] text-slate-500 font-bold mt-0.5">from ${Number(s.reviewCount || s.reviewsCount || 0)} verified buyers</div>
        </div>
        <div class="card p-4 text-center">
          <div class="text-[10px] uppercase font-bold tracking-wider text-slate-400">Dispatch Speed</div>
          <div class="font-display font-extrabold text-2xl text-[#081F5C] mt-1">${esc(s.processingTime || 'Not configured')}</div>
          <div class="text-[10px] text-slate-500 font-bold mt-0.5">Seller Dispatch Location</div>
        </div>
        <div class="card p-4 text-center">
          <div class="text-[10px] uppercase font-bold tracking-wider text-slate-400">In-State Delivery</div>
          <div class="font-display font-extrabold text-2xl text-emerald-600 mt-1">${s.freeShippingInState ? 'FREE' : (s.sameStateFee != null ? fmt(s.sameStateFee) : 'Not configured')}</div>
          <div class="text-[10px] text-slate-500 font-bold mt-0.5">Seller orders</div>
        </div>
      </div>
        ...
        <div class="grid sm:grid-cols-3 gap-4 text-xs">
          <div class="p-3.5 rounded-2xl bg-[#EEF3FF]/70 border border-[#D0E3FF]">
            ...
            <p class="text-slate-600 text-[11px] mt-1.5 leading-relaxed">
              <strong>Delivery policy:</strong> Shipping availability and fees are configured by the seller.
            </p>
          </div>
          <div class="p-3.5 rounded-2xl bg-[#EEF3FF]/70 border border-[#D0E3FF]">
            <div class="flex items-center gap-2 font-bold text-[#081F5C]">
            ...
            <p class="text-slate-600 text-[11px] mt-1.5 leading-relaxed">
              ${esc(s.returnPolicy || 'Return policy not configured by this seller.')}
            </p>
          </div>
          <div class="p-3.5 rounded-2xl bg-[#EEF3FF]/70 border border-[#D0E3FF]">
            <div class="flex items-center gap-2 font-bold text-[#081F5C]">
            ...
            <p class="text-slate-600 text-[11px] mt-1.5 leading-relaxed">
              Seller-provided authenticity information is shown here when configured. Buyer protection and payment settlement are applied according to the marketplace checkout policy.
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="mt-10">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
          ...
          <div class="flex items-center gap-2 flex-wrap">
            ...
            <button class="px-3 py-1.5 rounded-xl bg-[#081F5C] text-white text-xs font-bold" data-action="seller-tab" data-tab="listings">All Items (${products.length})</button>
            <button class="px-3 py-1.5 rounded-xl bg-white border border-slate-200 text-slate-700 text-xs font-bold hover:bg-slate-50" data-action="seller-tab" data-tab="listings">Figures</button>
            <button class="px-3 py-1.5 rounded-xl bg-white border border-slate-200 text-slate-700 text-xs font-bold hover:bg-slate-50" data-action="seller-tab" data-tab="listings">Apparel</button>
          </div>
        </div>
            ...
            return `
              <div class="prod-card card !rounded-3xl p-3.5 flex flex-col hover:shadow-lg transition group">
                ...
                <div class="pt-3 flex-1 flex flex-col">
                  <div class="text-xs font-bold text-[#334EAC]">${esc(p.brand || '—')}</div>
                  ...
                  <div class="mt-auto pt-3 flex items-end justify-between border-t border-slate-100">
                    <div>
                      ...
                      <div class="flex items-center gap-1 text-[10px] text-slate-500 font-bold">
                        ...
                        <span>${Number(p.rating || 0)} (${Number(p.reviews || p.reviewsCount || 0)})</span>
                      </div>
                    </div>
...
function viewSeller(){
    ...
    <section class="grid xl:grid-cols-[1.4fr_0.9fr] gap-5">
      <div class="rounded-3xl border border-[#E5E7EB] bg-white p-5 shadow-sm">
        <div class="flex items-center justify-between mb-4">
          ...
          <button class="btn btn-ghost !px-3 !py-2 !text-[11px]" data-action="seller-tab" data-tab="analytics">Last 30 days</button>
        </div>
  ...
  const paymentsContent = `<div class="card p-5"><div class="text-[10px] uppercase tracking-[.18em] text-[#64748B] font-bold">Earnings</div><h2 class="font-display font-extrabold text-2xl mt-1">Financial overview</h2><p class="mt-3 text-sm text-[#64748B]">Financial data will appear after real marketplace orders and settlements are recorded.</p></div>`;

  const reviewsContent = `<div class="rounded-3xl border border-[#E5E7EB] bg-white p-5 shadow-sm"><div class="text-[10px] uppercase tracking-[.18em] text-[#64748B] font-bold">Reviews</div><h2 class="font-display font-extrabold text-2xl text-[#111827] mt-1">Seller rating</h2><div class="mt-4 flex items-center gap-4"><div class="font-display font-extrabold text-4xl text-[#111827]">0.0</div><div class="text-[#E9B949] flex"><i data-lucide="star" style="width:16px;height:16px;fill:#E9B949;stroke:#E9B949"></i><i data-lucide="star" style="width:16px;height:16px;fill:#E9B949;stroke:#E9B949"></i><i data-lucide="star" style="width:16px;height:16px;fill:#E9B949;stroke:#E9B949"></i><i data-lucide="star" style="width:16px;height:16px;fill:#E9B949;stroke:#E9B949"></i><i data-lucide="star" style="width:16px;height:16px;fill:#E9B949;stroke:#E9B949"></i></div></div><div class="mt-5 rounded-2xl border border-dashed border-[#D1D5DB] bg-[#F8FAFC] p-6 text-center text-sm text-[#64748B]">Review summaries, customer feedback and seller replies will appear here.</div></div>`;

  const analyticsContent = `<div class="card p-5"><div class="text-[10px] uppercase tracking-[.18em] text-[#64748B] font-bold">Analytics</div><h2 class="font-display font-extrabold text-2xl mt-1">Store performance</h2><p class="mt-3 text-sm text-[#64748B]">Analytics are calculated from real seller activity and will populate as orders and views occur.</p></div>`;
  ...
  function getSellerSettings(){
    const defaults = {
      account: { firstName:'', lastName:'', displayName:'', email:'', phone:'', password:'', profileImage:'', emailVerified:false },
      store: { name:'', description:'', category:'', contact:'', policies:'', logo:'', banner:'' },
      location: { country:'Nigeria', state:'', city:'', dispatch:'' },
      ...
      shipping: { local:true, national:true, pickup:false, free:false, localFee:'0', nationalFee:'0', dispatch:'', processing:'', options:[] },
      returns: { window:'Marketplace default', conditions:{unused:true, packaging:true, purchase:true}, locked:true },
      payout: { method:'Bank transfer', schedule:'', bank:'', accountNumber:'', verified:false },
      fees: { commission:15, transaction:'₦0', refund:'Policy-based' },
      ...
      security: { twoStep:false, sessions:[] },
      verification: { status:'Not Started', step:0 },
  ...
  function renderSellerSettingsPage(){
    ...
    const seller = sellerOf(user ? user.id : null);
    if(!seller) return '<div class="card p-8 text-center"><p class="text-sm text-[#708BD1]">Seller identity not found.</p></div>';
    ...
    const payoutMasked = settings.payoutAccounts && settings.payoutAccounts[0] ? settings.payoutAccounts[0].masked : 'Not configured';
          ...
          <div class="space-y-5">
            ...
            <div id="settings-tax-fees" class="rounded-3xl border border-[#E5E7EB] bg-white p-5 shadow-sm" data-section="Taxes & Fees">
              ...
              <div class="mt-5 grid md:grid-cols-2 gap-4">
                ...
                <div class="rounded-2xl bg-[#F8FAFC] border border-[#E5E7EB] p-4"><div class="text-[10px] uppercase tracking-[.15em] text-[#64748B] font-bold">Transaction fee</div><div class="font-display font-extrabold text-2xl mt-2 text-[#111827]">₦0</div></div>
    ...
    <div class="max-w-[1600px] mx-auto px-4 py-4">
      <header class="rounded-full border border-[#E5E7EB] bg-white/80 backdrop-blur-xl px-4 py-3 shadow-sm sticky top-3 z-20">
        <div class="flex items-center justify-between gap-3 flex-wrap">
          ...
          <div class="flex items-center gap-2"><button class="btn btn-ghost !px-3 !py-2 !text-[11px]" data-settings-action="preview-store">Preview shop</button><button class="btn btn-primary !px-3 !py-2 !text-[11px]" data-action="seller-tab" data-tab="listings">Add listing</button></div>
        </div>
      </header>
        ...
        <main class="space-y-5">
          <section class="rounded-3xl border border-[#E5E7EB] bg-white p-5 shadow-sm">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
              ...
              <div class="flex flex-wrap items-center gap-2">
                <button class="btn btn-ghost !px-3 !py-2 !text-[11px]" data-action="seller-tab" data-tab="earnings">Sales report</button>
...
function saveSettingsSection(section){
  ...
  if(section === 'account'){
    ...
    settings.account.firstName = (data.get('firstName') || '').trim();
    settings.account.lastName = (data.get('lastName') || '').trim();
    settings.account.displayName = (data.get('displayName') || '').trim();
  ...
  if(section === 'store'){
    const data = new FormData(form);
    settings.store.name = (data.get('name') || '').trim();
    settings.store.description = (data.get('description') || '').trim();
    settings.store.category = (data.get('category') || 'Figures & Collectibles');
    settings.store.contact = (data.get('contact') || '').trim();
    settings.store.policies = (data.get('policies') || '').trim();
    if(settings.store.description.length < 20){ toast('Store description must be at least 20 characters.', 'alert-circle'); return; }
  }

  if(section === 'location'){
    const data = new FormData(form);
    settings.location.state = data.get('state') || '';
    settings.location.city = data.get('city') || '';
    settings.location.dispatch = (data.get('dispatch') || '').trim();
  }
  ...
  if(section === 'shipping'){
    ...
    settings.shipping.localFee = (data.get('localFee') || '0').toString();
    settings.shipping.nationalFee = (data.get('nationalFee') || '0').toString();
    settings.shipping.dispatch = (data.get('dispatch') || '').trim();
  ...
  if(section === 'payout'){
    ...
    settings.payout.bank = (data.get('bank') || '').trim();
    settings.payout.accountNumber = (data.get('accountNumber') || '').trim();
  }
  ...
  if(section === 'all'){
    ...
    sections.forEach(key => {
      ...
      if(key === 'account'){
        merged.account.firstName = (formData.get('firstName') || '').trim();
        merged.account.lastName = (formData.get('lastName') || '').trim();
        merged.account.displayName = (formData.get('displayName') || '').trim();
      ...
      if(key === 'store'){
        merged.store.name = (formData.get('name') || '').trim();
        merged.store.description = (formData.get('description') || '').trim();
        merged.store.category = (formData.get('category') || 'Figures & Collectibles');
        merged.store.contact = (formData.get('contact') || '').trim();
        merged.store.policies = (formData.get('policies') || '').trim();
      }
      if(key === 'location'){
        merged.location.state = formData.get('state') || '';
        merged.location.city = formData.get('city') || '';
        merged.location.dispatch = (formData.get('dispatch') || '').trim();
      }
      ...
      if(key === 'shipping'){
        ...
        merged.shipping.localFee = (formData.get('localFee') || '0').toString();
        merged.shipping.nationalFee = (formData.get('nationalFee') || '0').toString();
        merged.shipping.dispatch = (formData.get('dispatch') || '').trim();
...
document.addEventListener('click', (event) => {
  ...
  if(action === 'preview-store'){
    const settings = getSellerSettings();
    const seller = currentUser() && sellerOf(currentUser().id); window.alert(`Store preview\n\n${settings.store.name}\n${settings.store.description}\n${settings.location.city}, ${settings.location.state}\n${seller?.status === 'approved' ? 'Approved seller' : 'Not publicly approved'}`);
  }
  ...
  if(action === 'save-password'){
    ...
    if(password.length < 8){ toast('Password must be at least 8 characters long.', 'alert-circle'); return; }
    const u = currentUser();
    if(!u) return;
    const users = LS.get('users', []);
    const i = users.findIndex(x => x.id === u.id);
    if(i < 0) return;
    users[i].password = password; users[i].updatedAt = Date.now(); LS.set('users', users);
    settings.account.password = ''; saveSellerSettings(settings);
    toast('Password saved.', 'lock-keyhole'); route();
  }
  ...
  if(action === 'fee-breakdown'){
    const settings = getSellerSettings();
    window.alert(`Fee breakdown\n\nCommission rate: ${settings.fees.commission}% (admin controlled)\nShipping: ₦${settings.shipping.localFee || '0'}\nRefund adjustment: ${settings.fees.refund}\nActual earnings are calculated from completed marketplace orders.`);
  }
  if(action === 'payout-add'){
    const bank = prompt('Enter payout bank name');
    const account = prompt('Enter 10-digit account number');
...
document.addEventListener('change', (event) => {
  const upload = event.target.closest('[data-settings-action=\"upload-logo\"], [data-settings-action=\"upload-banner\"]');
  if(upload){
    const file = upload.files && upload.files[0];
    if(!file) return;
    if(!file.type.startsWith('image/')) { toast('Please select an image file.', 'alert-circle'); return; }
    if(file.size > 2 * 1024 * 1024) { toast('Image must be 2 MB or smaller.', 'alert-circle'); return; }
    const reader = new FileReader();
    reader.onload = () => { const settings=getSellerSettings(); if(upload.dataset.settingsAction==='upload-logo') settings.store.logo=reader.result; else settings.store.banner=reader.result; saveSellerSettings(settings); updateSellerProfileFromSettings(settings); toast(`${upload.dataset.settingsAction==='upload-logo'?'Store logo':'Store banner'} uploaded and saved.`, 'image'); route(); };
...
document.addEventListener('change', (event) => {
  const photo = event.target.closest('[data-profile-image]');
  if (photo) {
    const file = photo.files && photo.files[0];
    if (!file) return;
    if (!file.type.startsWith('image/')) { toast('Please select an image file.', 'alert-circle'); return; }
    if (file.size > 2 * 1024 * 1024) { toast('Profile image must be 2 MB or smaller.', 'alert-circle'); return; }
    const reader = new FileReader();
    reader.onload = () => {
      const u = currentUser();
      if (!u) return;
      ...
      if (idx < 0) return;
      users[idx].profilePicture = reader.result;
      users[idx].updatedAt = Date.now();
      LS.set('users', users);
      toast('Profile photo uploaded and saved.', 'image');
      route();
    };
    reader.readAsDataURL(file);
    return;
...
function sellerApply(formEl){
  ...
  if(marketplaceSettings.sellerRegistrationEnabled === false){ toast('Seller registration is currently closed by ANILyfe administration.','store'); return; }
  const sellers = LS.get('sellers', []);
  if(sellers.some(s=>s.userId===u.id)){ toast('A seller application already exists for this account.','store'); location.hash='#/seller/verification'; return; }
  sellers.push({
  ...
  LS.set('sellers', sellers);
  toast('Application submitted — complete verification before admin review.', 'hourglass');
  location.hash = '#/seller/verification';
}
