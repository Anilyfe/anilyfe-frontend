...
function viewLanding(){
  ...
  <div class="min-h-screen hero-ambient relative overflow-hidden">
    ...
    ${(()=>{const a=(LS.get('announcements',[])||[]).find(x=>x.active!==false && !(LS.get('announcementDismissed',[])||[]).includes(x.id));return a?`<div class="relative z-20 bg-white/10 border-b border-white/10"><div class="max-w-7xl mx-auto px-5 py-2.5 flex items-center gap-3 text-white text-xs"><i data-lucide="megaphone" style="width:14px;height:14px;color:#E9B949"></i><span class="flex-1 font-bold">${esc(a.text)}</span>${a.link?`<a href="${esc(a.link)}" target="_blank" rel="noopener" class="text-[#E9B949] font-extrabold">Open</a>`:''}<button data-action="dismiss-announcement" data-id="${a.id}" class="text-white/70">✕</button></div></div>`:''})()}
    <!-- nav -->
        ...
        <div class="glass rounded-3xl p-8 md:p-14 grid md:grid-cols-2 gap-10 items-center reveal">
          ...
          <div class="relative">
            <div class="rounded-2xl overflow-hidden border-4 border-white shadow-2xl rotate-2 hover:rotate-0 transition duration-500">
              <img src="" alt="Anime figures" class="w-full h-72 object-cover" onerror="this.parentElement.style.background='linear-gradient(150deg,#D0E3FF,#708BD1)';this.remove()"/>
            </div>
