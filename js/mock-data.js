...
/* ---------- seed data ---------- */
function seed(){
  const existingSellers = LS.get('sellers', []);
  ...
  const abyssSeller = {
    ...
    slug: 'abyss-atelier',
    sells: 'Official Masterpiece Anime Figures & Cyber Streetwear',
    startingPrice: 5000,
    status: 'approved',
    rating: 4.9,
    sales: 142,
    ...
    badges: ['Founding Seller #001', 'Verified Seller'],
  ...
  const defaultProductsList = [
    P('PRD-LFY5','SLR-001','Luffy Gear 5 Masterpiece Figure',25000,12,'Figures & Collectibles',IMG.fig1,4.9,38,10),
    P('PRD-GOJO','SLR-001','JJK Domain Expansion Cyber Hoodie',18000,25,'Clothing & Apparel',IMG.hood1,4.9,24,15),
    P('PRD-NRTS','SLR-001','Naruto Sage Mode Dual Katana Set',22000,8,'Figures & Collectibles',IMG.fig2,4.8,19,0),
  ...
  if (!existingSellers.length || !existingSellers.some(s => s.id === 'SLR-001')) {
    LS.set('sellers', defaultSellers);
  }
  if (!existingProducts.length || !existingProducts.some(p => p.id === 'PRD-LFY5')) {
    LS.set('products', defaultProductsList);
  }
  if (!LS.get('admins')) {
  ...
  if (!LS.get('dealEnd')) LS.set('dealEnd', Date.now() + (2*86400 + 14*3600 + 35*60 + 42)*1000);
  LS.set('seeded', true);
}
