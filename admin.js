...
function getAdminMetrics(){
  ...
  const approvedProducts = products.filter(p => ['approved','Published','Approved'].includes(p.approvalStatus || p.status)).length;
  ...
  return `
    <div class="min-h-screen bg-[#F6FCFF] flex">
      ...
      <div class="flex-1 min-w-0">
        ...
        ${adminProductPreviewId ? renderAdminProductPreview(adminProductPreviewId, metrics) : ''}
      </div>
    </div>
  `;
}
