...
(function() {
  ...
  document.addEventListener('click', async function(e) {
    ...
    if (act === 'sc-product-edit') {
      ...
      if (name === null) return;
      const price = prompt('Price (NGN):', String(product.price ?? ''));
      if (price === null) return;
      const stock = prompt('Stock quantity:', String(product.stock ?? 0));
      if (stock === null) return;
      try {
        await window.productService.updateProduct(id, { name:name.trim(), price:Number(price), variants:(product.variants || [{sku:product.sku || '', stock:Number(stock) || 0, price:Number(price) || 0}]) });
        toast('Product updated. Changes may require re-approval before publishing.', 'check-circle-2'); route();
    ...
    if (act === 'sc-wizard-add-sample-photo') {
      const url = prompt('Enter anime photo image URL:',);
      if (url) {
